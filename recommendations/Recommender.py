...
class Recommender:
    ...
    def find_most_significant_article(self, articles, liked_article):
        ...
        for article in articles:
            ...
            if num_articles_with_score_above_trehshold>50:
                break 
    ...
    def generate_recommendations(self, user):
        ...
        for article_liked in articles_liked_by_user:
            for similar_article in article_liked.similar_articles.all()[:10]:
                recommendation = Recommendation.objects.filter(article = similar_article.related_article, user = user).first()
    ...
    def generate_recommendations_based_on_one_article(self, current_article):
        ...
        for article in all_articles:
            similarity_score = self.cosine_similarity_score(current_article.text_vector,article.text_vector)
            
            if similarity_score > self.threshold:
                #print(similarity_score)
                similar_article = SimilarArticle.objects.filter(principal_article=current_article,related_article=article).first()
                if not similar_article:
                    
                    similar_article = SimilarArticle(principal_article=current_article,related_article = article)
        ...
        for idx in range(2224,length):
            ...
            for article_a in all_articles:
                if all_articles[idx] != article_a.id:
                    sim_score = self.cosine_similarity_score(all_articles[idx].text_vector,article_a.text_vector)
                    similar_article = SimilarArticle.objects.filter(principal_article=all_articles[idx],related_article=article_a).first()
